...
library(ranger)
#model_rf <- ranger(Site_Code ~ Pat_X + Pat_Y + Site_Type + Month + Year + Age_Group + Attendance_Type, data = train, importance = "impurity")
...
importance_values_site <- model_rf$variable.importance

# Create a data frame for plotting
importance_df <- data.frame(
  Variable = names(importance_values_site),
  Importance = importance_values_site
)
...
ggplot(importance_df, aes(x = reorder(Variable, Importance), y = Importance)) +
  ...
  coord_flip() +
  labs(title = "Feature Importance for Site Classification Random Forest", x = "Variable Name", y = "Importance") +
  theme_minimal()
